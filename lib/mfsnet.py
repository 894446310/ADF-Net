...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth=26, scale=4, num_classes=1000):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 32, 3, 2, 1, bias=False),#in,out,kernel size,stide,padding
            nn.BatchNorm2d(32),
...
class aggregation(nn.Module):
    ...
    def forward(self, x1, x2, x3):  #2,32,7,7   #2,32,14,14  #2,32,28,28
        x1_1 = x1
...
class MFSNet(nn.Module):
    ...
    def forward(self, x):
        a1=x
        x = self.resnet.conv1(x) #3.64.2倍下采样
        a2=x
        x = self.resnet.bn1(x)
        x = self.resnet.relu(x)
        x = self.resnet.maxpool(x)#    2倍下采样
        a3=x# bs, 64, 88, 88
        # ---- low-level features ----
        x1 = self.resnet.layer1(x) #64，256 # bs, 256, 88, 88
        a4=x1
        x2 = self.resnet.layer2(x1) #通道加倍，2倍下采样 # bs, 512, 44, 44
        a5=x2
        # ---- high-level features ----
        # x2 = self.resnet.layer2(x1)     # bs, 512, 44, 44
        x3 = self.resnet.layer3(x2)#通道加倍，2倍下采样  # bs, 1024, 22, 22
        a6=x3
        x4 = self.resnet.layer4(x3)#通道加倍，2倍下采样  # bs, 2048, 11, 11
        a7=x4
        x2_rfb = self.rfb2_1(x2)  # channel -> 32
        x3_rfb = self.rfb3_1(x3)  # channel -> 32
        x4_rfb = self.rfb4_1(x4)  # channel -> 32

        x = self.edge_conv1(x1)   #256，64
        a8=x
        x = self.edge_conv2(x)    #64,64
        edge_guidance = self.edge_conv3(x) # 64,64wubianhua  # torch.Size([1, 64, 88, 88])
        lateral_edge = self.edge_conv4(edge_guidance) #2,1,224,224 # NOTES: Sup-2 (bs, 1, 88, 88) -> (bs, 1, 352, 352)
        a9=lateral_edge
...
#                     f.endswith('.tif') or f.endswith('.png') or f.endswith('.PNG')]
#         self.images = sorted(self.images)
#         self.gts = sorted(self.gts)

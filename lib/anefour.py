...
class MiT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x0=x
        B = x.shape[0]   #取出batch size
        outs = []

        # stage 1
        x, H, W = self.patch_embed1(x)    #2,HxW,C    2，3136，128
        ax=x
        for blk in self.block1:
            x = blk(x, H, W)
        x = self.norm1(x)    #改通道的，但是没变
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()   #序列变成二维
        outs.append(x)
...
if __name__ == '__main__':
    ...
    #     print(_)
    # aa=([DualTransformerBlock(in_dim[0], key_dim[0], value_dim[0], head_count, token_mlp) for i in range(1, 11)])
    # a=[DualTransformerBlock(in_dim[0], key_dim[0], value_dim[0], head_count, token_mlp) for _ in range(layers[0])]
    # model = RCEM(3, 64)

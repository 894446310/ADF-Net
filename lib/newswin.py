...
class up4_conv(nn.Module):                                           #上采样
    def __init__(self,ch_in,ch_out):
        super(up4_conv,self).__init__()
        self.up = nn.Sequential(
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1,
        ...
        self.Up4 = up4_conv(ch_in=96, ch_out=64)
        ...
        for i_layer in range(self.num_layers):      #range为4
    ...
    def forward(self, x):
        # x: [B, L, C]
        d1=x
        #d111=x.flatten(2).transpose(1, 2)
        #d112=d111.view(-1, 224, 224, 3).permute(0, 3, 1, 2).contiguous()
        ...
        f3 = self.norm2(o2)
        f4 = self.norm(o3)
        b1 = f1.view(-1, 56, 56, 96).permute(0, 3, 1, 2).contiguous()#f1.view(-1, 56, 56, 96).permute(0, 3, 1, 2).contiguous()
        b2 = f2.view(-1, 28, 28, 192).permute(0, 3, 1, 2).contiguous()
        b3 = f3.view(-1, 14, 14, 384).permute(0, 3, 1, 2).contiguous()
        b4 = f4.view(-1, 7, 7, 768).permute(0, 3, 1, 2).contiguous()
